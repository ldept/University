...
double process_middle(const point* midpoints,const int &n,const double &dist){
    double d = dist;
    for(int i =0; i<n; i++){
...
double dothework(const point* sortedX,const point* sortedY,const int &n){
    //if(n == 2){
    //    P1 = sortedX[0];
    //    P2 = sortedY[1];
    ...
    if (n <= 3){
        ...
        double leftdist = dothework(sortedX,sortedLeft,middle+1);
        double rightdist = dothework(sortedX + middle+1,sortedRight,n-middle-1);
...
double closest_pair(const point* p,const int &n){
