...
(define (heap-merge h1 h2)
  (cond
   ...
   [(leaf? h2) h1]
   ;; XXX: fill in the implementation
   [else (if (< (elem-priority (heap-min h1)) (elem-priority (heap-min h2)))
             (make-node (heap-min h1)
                        (heap-merge (node-right h1) h2)
                        (node-left h1))
             (make-node (heap-min h2)
...
(module+ test
  (require rackunit)
  (check-true (sorted? (heapsort (randlist 1000 23124))))
  (check-true (sorted? (heapsort (randlist 1000 23124))))
  (check-true (sorted? (heapsort (randlist 1000 23124))))
  (check-true (sorted? (heapsort (randlist 1000 23124))))
  (check-true (sorted? (heapsort (randlist 1000 23124))))
  ...
  (define test-list (randlist 10 24))
  test-list
  (heapsort test-list)
  (sorted? (heapsort test-list)))
