...
(define (solve-ax+by=1 a b)
  ...
  ;;; Na podstawie rozszerzonego algorytmu euklidesa
  (if (= a 0)
      (cons 0 1)
      (let ([x (solve-ax+by=1 (modulo b a) a)])
      (let ([p1 (car x)]
            [p2 (cdr x)])
        (cons (- p2 (* (floor (/ b a)) p1))
              p1)))))
...
(define (RSA-unconvert-list intlist key)
  ...
  (let ((n (key-modulus key)))
    (define (unconvert l sum)
      (if (null? l)
          '()
          (let ((x (modulo (+ [RSA-transform (car l) key] sum)
                           n)))
...
(define (encrypt-and-sign message recipient-public-key sender-private-key)
  ;;; XXX: Write your code here
   (make-signed-message (RSA-transform (compress(RSA-encrypt message recipient-public-key)) sender-private-key)
                        (RSA-encrypt message recipient-public-key)))
  

(define (authenticate-and-decrypt cyphertext recipient-private-key sender-public-key)
  ;;; XXX: Write your code here
  (if (= (RSA-transform (signature cyphertext) recipient-private-key) (compress(RSA-encrypt (message cyphertext) sender-public-key)))
      #t
      #f))
