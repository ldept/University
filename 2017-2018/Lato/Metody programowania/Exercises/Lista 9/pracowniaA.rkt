...
(define fermat-test
  '{
    ...
    (while (> k 0)
           ( (a := (+ 2 (rand (- n 3)))) ;ustalamy a od 2 do n-2
             (i := (- n 1)) ;licznik do podniesienia a do potegi n-1
             (expt := a) ;podstawa potegowania
             (while (> i 1) ;potegowanie a do n-1 
                    ( (a := (* a expt))
                      (i := (- i 1))))
             (if (not (= (mod a n) 1)) ;czy zlozona
                 ( (composite := true)
                   (k := 0)) ;jesli tak - zakoncz algorytm
                 (k := (- k 1))) ;wpp kontynuuj
             ))}
...
(module+ test
  (require rackunit) 
  (check-equal? (probably-prime? 4 100) #f)
  (check-equal? (probably-prime? 3 21) #t)
  (check-equal? (probably-prime? 13 33) #t)
  (check-equal? (probably-prime? 97 21) #t)
  (check-equal? (probably-prime? 1223 123) #t)
  (check-equal? (probably-prime? 26 21) #f))