...
(define eval-binop
  '(lambda-rec (rec expr)
               (if (const? expr)
                   ...
                   (cond ((pair? expr)
                          (cond ([(eq? '+ (car expr))
                                  (+ (rec (car (cdr expr))) (rec (car (cdr (cdr expr)))))]
                                 [(eq? '- (car expr))
                                  (- (rec (car (cdr expr))) (rec (car (cdr (cdr expr)))))]
                                 [(eq? '* (car expr))
                                  (* (rec (car (cdr expr))) (rec (car (cdr (cdr expr)))))]
...
(module+ tests
  (require rackunit)
  (and (eq? (eval '(+ 3 4)) (eval (list eval-binop '(+ 3 4))))
       (eq? (eval '(- 3 (/ 4 2) (* (+ 1 2) 2)))
            (eval (list eval-binop '(- 3 (/ 4 2) (* (+ 1 2) 2)))))
       (eq? (eval 5) (eval (list eval-binop 5)))))